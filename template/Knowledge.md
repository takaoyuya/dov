...
- チーム体制（マネージメント、同僚のキャリア：いつからいつまで）
- 開発体制（ウォータフォール・アジャイル）
- マスタスケジュール
...
□Process
- Git ブランチ戦略（並行開発時のブランチ名チェック、タグルールも）
- プロセス
    - インシデントマネージメントプロセス：
    - レビュープロセス：
    - 通報・通知プロセス
...
- コンテンツ
    ...
    - コーディングガイドライン(言語ごと)
    - UTガイドライン：（関数命名ルール：Test{*テスト対象*}_{条件}_{*想定結果*}とう）
    - 非機能要件（ブラウザサポート/パフォーマンス等）
    - 閉局：SORRYページ（base64でリソースを埋め込む（CSS・画像ファイル）とlambdaのContent-lengthの上限オーバーでエラー注意）
    - インフラ
    - DB
        - トランザクション分離レベル
        - 文字コード
...
■資料テクニック
- 参加者を確認する：to all, ではなくto someone
- %（パーセント）より。100人に一人の方がインパクトが大きい
- 超一言：伝えたいことを一言のキーワードで表現する（キチンクルン）
