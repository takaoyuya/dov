...
### Writing format
- 読者：TO ALLではなくTO SOMEONE
- 目的
- ドキュメントのゴール(目的：だれが何を達成する為か)を設定する
    - 〜な人が対象です/〜をするひと対象で（WHO：NHKで分析）
    - 〜ができるようになる（GOAL：ユーザーストーリー）
- 主張
- 内容：ユースケースやケーススタディも追加する
- サマリ
...
#### レビュー観点
- （最重要）スタンドアロンか：口頭補足必要な資料はナレッジとして価値低い
- つくることが目的化はNG（不要であれば作らない）
- メッセージを体言止めにしない：体言止めは見出しになりやすい。メッセージになっていない（バーバラミント）
- 曖昧言葉つかわない：見直す。再構築、問題がある、適切に
- 具体と抽象両方記載あるか（例をつけるなど）
- 割れた窓を放置しない：前回からのミスも合わせて修正する
- 知識の呪い：他の人がじぶんと同じ知識を持っていると思い込むバイアス
- 完璧とは何にも追加するものがないときではなく、何も取り去るべきもの無くなった時（サンテグジュベリ：フランスの作家）
...
#### SECI　MODEL：知識変換のスパイラル型フレームワーク
![SECI](/images/seci-model.JPG)
...
#### DIKW　MODEL：ナレッジレベルのピラミッド型フレームワーク
![DIKW](/images/DIKW.JPG)

- ナレッジレベル
  - データ（Data）
    - 単体ではナレッジとしては扱えず、価値が見出せていない存在がデータであり、そのまま蓄積しても活用できない。
      - （例：数字１０、キーワードなど）

  - 情報（Information）
    - データが意味を持つように情報を加える。
    （例：「10」→「Aさんの1日の塩分摂取量は10g」）

  - 知識（Knowledge）
    - 知識とは主観（解釈）を含み、これまでの経験など固有情報を加える。
    （例：「Aさんの1日の塩分摂取量は10g」→「Aさんの1日の塩分摂取量10gは厚生労働省推奨の塩分摂取量7.5g未満よりも多い」）

  - 知恵（Wisdom）
    - 正しい評価・判断が実施されて、行動に移せる状態。
    （例：→「Aさんの1日の塩分摂取量10gは厚生労働省推奨の塩分摂取量7.5g未満よりも多いため、最低でも毎日2.5gの塩分を減らす必要がある」）
